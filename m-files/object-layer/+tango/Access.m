classdef Access < handle
    ...
    methods(Static,Access=protected,Hidden)
        function attr=attrdefine(devname, attrinfo,pollperiod)
            is_enum = false;
            ...
            if size(attrinfo.enum_labels, 1) ~= 0
            	attr.is_enum = true;
            	attr.enum = tango.DevEnum(devname, attr.name);
            else
            	attr.is_enum = false;
            	attr.enum = [];
            end
        ...
        function val=attrprocess(attrinfo,reply)
            ...
            if attrinfo.is_enum
                val=struct(...
                    ...
                    'set_label',{attrinfo.enum.value2label(attrinfo.conversion(reply.setpoint))},...
                    'read',{attrinfo.conversion(reply.value)},...
                    'read_label',{attrinfo.enum.value2label(attrinfo.conversion(reply.value))},...
                    'labels',{attrinfo.conversion(attrinfo.enum.labels)},...
                    'time',{reply.time},...
